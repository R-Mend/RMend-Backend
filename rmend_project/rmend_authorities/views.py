...
from .serializers import AuthorityIssueTypeGroupSerializer
from .permissions import IsAuthorityAdmin


class IssueTypeGroupView(APIView):
    """API view for getting issue type groups by general users"""
    permission_classes = [AllowAny]

    def post(self, request):
        """Gets a list of issue type groups based on users location"""
        location = request.data.get('location')
        if not location:
            return missing_requred_data_error('location')

        pnt = Point(location[0], location[1])
        issue_type_groups = AuthorityIssueTypeGroup.objects.filter(
            authority__report_range__touches=pnt)
...
class AuthorityIssueTypeGroupView(APIView):
    """API view for admin get access to issue type groups"""
    permission_classes = [IsAuthenticated, IsAuthorityAdmin]

    def get(self, request, *args, **kwargs):
        """Gets a list of IssueTypeGroups based usrs authority"""
...
class AuthorityIssueTypeGroupCreateView(APIView):
    """API view for IssueTypeGroup creation"""
    permission_classes = [IsAuthenticated, IsAuthorityAdmin]
...
class AuthorityIssueTypeGroupDeleteView(APIView):
    """API view for IssueTypeGroup deletion"""
    permission_classes = [IsAuthenticated, IsAuthorityAdmin]
    ...
    def delete(self, request, authority_id):
        ...
        try:
            issue_group = AuthorityIssueTypeGroup.objects.get(
                name=group_name, authority__id=authority_id)
...
class AuthorityIssueTypeCreateView(APIView):
    """API view for IssueType creation"""
    permission_classes = [IsAuthenticated, IsAuthorityAdmin]

    def post(self, request, authority_id):
        """Creates a new issue type for the requested authority"""
        # Verify that all the required info is given
        ...
        if not issue_group_name or not type_name:
            return missing_requred_data_error(
                'issue_group' if not issue_group_name else 'type_name')

        # Get the requested issue type group to add a new issue type to
        try:
            issue_group = AuthorityIssueTypeGroup.objects.get(
                name=issue_group_name, authority__id=authority_id)
        except AuthorityIssueTypeGroup.DoesNotExist:
        ...
        # Get the base issue type group to copy into the authorities issue type groups
        try:
            base_issue_type = BaseIssueType.objects.get(
                name=type_name, issue_group__name=issue_group_name)
...
class AuthorityIssueTypeDeleteView(APIView):
    """API view for IssueType deletion"""
    permission_classes = [IsAuthenticated, IsAuthorityAdmin]

    def delete(self, request, authority_id):
        """Deletes the requested reports if the user is an admin of the reports authority"""
         # Verify that all the required info is given
        type_name = request.data.get('type_name')
        ...
        try:
            issue_type = AuthorityIssueType.objects.get(
                name=type_name, issue_group__authority=authority_id)
...
def data_already_exist_error(data_type, data):
    """Returns a response error for when data is trying to be created but already exist"""
